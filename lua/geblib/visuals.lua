...
loadInternal("geblib/visuals_wave.lua")(Visuals, Runtime, Surface, Config)
...
function Visuals.CreateWaterDebris(position, normal, strength, options)
    ...
    if options.effects ~= false then
        ...
        for index = 1, splashCount do
            ...
            if options.gunshotSplashes ~= false
                and waterEffect("gunshotsplash", effectPosition, normal, scale)
            then
                spawned = spawned + 1
            end
        end
