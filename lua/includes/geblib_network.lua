...
if CLIENT then
	local function DebugMessage(len, ent, varName, value)
		if gebLib.DebugMode() and gebLib.NetworkDebug() then
			...
			print("Client: " ..
				...
				tostring(varName) ..
				", variable value: " ..
				tostring(value) .. ", indexed: " .. tostring(isnumber(index)) .. ", index: " .. tostring(index))
		end
	end
	...
	local function SetEntityValue(value, len)
		...
		if not ent:IsValid() then
			error("gebLib Networking: Trying to update variable: " ..
				varName ..
				" to value: " ..
				tostring(value) .. " on ent that is nil, ent might not be loaded or does not exist on the client!")
		end
